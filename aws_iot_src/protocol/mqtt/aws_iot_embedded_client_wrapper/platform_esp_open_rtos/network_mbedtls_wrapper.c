...
#ifdef MBEDTLS_DEBUG_C
	mbedtls_debug_set_threshold(4/*DEBUG_LEVEL*/);
...
int iot_tls_connect(Network *pNetwork, TLSConnectParams params) {
	...
	ret = mbedtls_x509_crt_parse(&cacert, AWS_IOT_ROOT_CA, strlen(AWS_IOT_ROOT_CA) + 1);
	if (ret < 0) {
	...
	ret = mbedtls_x509_crt_parse(&clicert, AWS_IOT_CERTIFICATE, strlen(AWS_IOT_CERTIFICATE) + 1);
	if (ret != 0) {
		ERROR(" failed\n  !  mbedtls_x509_crt_parse returned -0x%x\n\n", -ret);
		return ret;
	}

	ret = mbedtls_pk_parse_key(&pkey, AWS_IOT_PRIVATE_KEY, strlen(AWS_IOT_PRIVATE_KEY) + 1, "", 0);
	if (ret != 0) {
	...
	//DEBUG("(ssl.session)->ciphersuite is %d", (ssl.session)->ciphersuite);
